...
print ("Loading symbol  from ", symbol, input_dir)
...
try:
    ...
    pyplot.plot(pred.predicted_mean, color='orange')
    pyplot.fill_between(ci.index, ci.iloc[:, 0], ci.iloc[:, 1], color='k', alpha=.2)
    #pyplot.plot(confidence, color='orange')
    pyplot.show()
    ...
    output.to_csv(output_dir + symbol + '_sarimax_ci.csv')
